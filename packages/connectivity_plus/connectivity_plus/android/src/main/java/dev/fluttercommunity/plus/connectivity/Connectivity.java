...
/**
 * @param wifiManager Thêm WifiManager để lấy SSID
 */
public record Connectivity(ConnectivityManager connectivityManager, Context context) {
    ...
    List<String> getCapabilitiesList(NetworkCapabilities capabilities) {
        ...
        if (capabilities.hasTransport(NetworkCapabilities.TRANSPORT_WIFI) || capabilities.hasTransport(NetworkCapabilities.TRANSPORT_WIFI_AWARE)) {
            ...
            String wifiName = wifiHelper.getCurrentSsid();
            types.add(wifiName);
        }
